...
* (サーバー管理者のみ) サーバーは、「VSCreepy_server.jar」というものをダウンロードしてください。
* (サーバー管理者のみ) 勝利したらワールドリセットする必要がある(再起動する必要がある)ので注意です。(エラーが出てサーバーが停止します。)
* サーバーに入れたら、誰かが```/startgame```と打つまでお待ちください。**必ず偶数人でないといけません。**
...
* ```/startgame```とチャットに打つとゲームが始まります。**必ず偶数人でないといけません。奇数になるとゲームが終了します。**
* 全員が**ダイヤモンドを手に持つ**ことが目的です。(インベントリに入れるだけではだめです。みんなが持つことが目的です。)
...
* (Server admins only) Download **“VSCreepy_server.jar”**
